...
#region Inner classes
        ...
        private static object GetObjectFromMode(LazyThreadSafetyMode mode)
        {
            if (mode == LazyThreadSafetyMode.ExecutionAndPublication)
                ...
                return LazyHelpers.PublicationOnlySentinel;
            else if (mode != LazyThreadSafetyMode.None)
                throw new ArgumentOutOfRangeException(nameof(mode), "Invalid LazyThreadSafetyMode value");
        ...
        {
            return IsValueCreated ? Value.ToString() : "The value for current Lazy instance was not created yet";
        }
